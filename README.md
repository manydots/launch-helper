...
- 物品编码查看：解析 `Script.pvf` 中的 `stackable/stackable.lst`，映射展示物品 ID / 名称 / 引用路径，支持原版与 Guard（0x55 XOR）两种格式
...
5. 支持原版与 Guard（0x55 XOR）两种 PVF 包头格式，打开时自动识别并标注
...
- **格式识别**：顶部徽章区分「Guard格式」（`0x55 XOR` 头）与「原版格式」
