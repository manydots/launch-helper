...
- NPK 归档预览与编辑：`docs/npk-format.md`（JP `ImagePacks2` NPK/IMG 格式与加解密算法注册表；解析与重建层 `src/utils/npkTool.js`，界面 `src/components/NpkView.vue`，验证脚本 `test/npk-verify.mjs` 与 `test/npk-roundtrip.mjs`；编辑/保存沿用原有加密算法不变）。
