...
- 重打包归档只读解析层：`src/utils/pvfToolRepack.js`（REPACK/60CN，第五种归档形态：明文归档魔数 + 逐 dword ROR6 XOR 加密的重打包归档；归档内为 GBK 明文文本，无 token 流 / 字符串池 / GRPI 分组，`dataType` 恒为 0 走编辑器既有只读展示分支，保存链显式拦截，右键菜单修改类入口经 `isReadonlyArchive` 隐藏；编辑回写未实现——消费端仅有读取端可对照，取得可被消费端加载的写入端权威对照前不得实现，见 `docs/pvf-repack-format.md` §4）。编辑器加载以 `RepackPvfArchive.sniff` 魔数探测**优先**分派（27 字节内无歧义），先于既有 JP/TW 探测链。解析层接口对齐编辑器消费的全部方法：修改类查询恒空态、引用修复安全禁用、修改类操作显式抛错（对齐 TwPvfArchive 安全禁用语义）。格式定义与字节级验证见 `docs/pvf-repack-format.md`（脚本 `test/repack-verify.mjs`）；读取端对照为同级消费端仓库（本机对照副本 `../DFLegacy`，C# 实现，仅作对照参考，权威性低于本仓文档与字节级验证）。
