...
- NPK 归档预览与编辑：`docs/npk-format.md`（JP `ImagePacks2` NPK/IMG 格式与加解密算法注册表；解析与重建层 `src/utils/npkTool.js`，界面 `src/components/NpkView.vue`，验证脚本 `test/npk-verify.mjs` 与 `test/npk-roundtrip.mjs`；编辑/保存沿用原有加密算法不变；音频/视频条目（SoundPacks `.ogg`、`.avi`）与独立加密 AVI 走不转码预览，MPEG 编码 AVI 经 TS 封装由 `src/vendor/jsmpeg.min.js`（JSMpeg，MIT，单文件 IIFE，`?raw` 动态加载）软解播放，格式与预览行为见其 §6）。
...
**合规约束**
- **文档中不得出现现有网游或知名游戏的名称**（含文件名、目录名、文档正文；引用验证基线文件路径、历史对话记录时除外），一律以中性名称（如"台服文件"、"日服文件"）或本文档定义的技术称谓代替，避免侵权与敏感信息。此约束适用于 `docs/` 下所有文档及 AGENTS.md 自身。
...
- **验证入口**：本项目 `package.json` 仅有 `prettier format`，**无 lint / typecheck 脚本**；以 `test/` 脚本为唯一验证入口，不得以"无 lint"为由跳过自测。验证脚本无法运行时，询问用户验证命令并建议写入本文件。
...
- **回归验证文件**（固定基线，勿改动；仅本机回归用，CI 环境需另行配置）。路径按「文档路径脱敏（门控）」以相对形式书写，基线根目录各机器不同（定位失败时按「工作流前置」询问用户）：
    ...
    - NPK/AVI 回归样例（同基线根 `PVF/test/` 下）：`sprite_common_etc.NPK`（IMG 基线归档，`test/npk-verify.mjs` 默认目标）、`sounds_char_creator.npk`（SoundPacks 音频包）、`creator.avi`（纯视频加密 AVI，V/W 组样例）、`ATFighterGrappler.avi`（带 MP2 音频加密 AVI，W6 组样例）；清单与断言见 `docs/npk-format.md` §4。
...
- 提交前必须 `git status` / `git diff` 确认只暂存本任务相关文件，不得混入无关改动；禁止提交密钥、本地路径（如 `D:\dnf\...`）等敏感信息。
