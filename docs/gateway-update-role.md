...
权威端（网关/游戏服务端）只强制**取值范围与组合关系**；以下第 3.3/3.4 条的等级门槛为前端展示层约定，
...
### 3.3 等级-转职/觉醒门槛（86 版本展示层约定）

| 形态 | 判定 | 最低参考等级 |
|:--:|----------|:--:|
| 转职（`grow_first ≠ 0`） | 转职分支 | 15 |
| 一次觉醒（`grow_second = 1`） | 一觉 | 50 |
| 二次觉醒（`grow_second = 2`） | 二觉 | 75 |

常量定义：

```
TRANSFER_MIN_LEVEL = 15        // 转职等级门槛
AWAKEN_MIN_LEVEL = { 1: 50, 2: 75 }   // 一次/二次觉醒等级门槛
```

满足关系表达式（对最终形态校验）：

```
(grow_first === 0 || refLevel >= 15) &&
(grow_second === 0 || refLevel >= AWAKEN_MIN_LEVEL[grow_second])
```

不满足时提交被拦截，提示文案引导管理员将目标等级一并设至门槛之上（例如设定二次觉醒而参考等级 50 时，
提示需把等级改为 ≥75 后一并提交）。该常量如与运营口径不符，仅需调整本文件所述常量与 UI 文案。

### 3.4 等级与觉醒双向约束（仅变更时校验）

等级与转职/觉醒门槛为**双向**约束：凡本次提交会导致**等级或转职/觉醒任一变化**，
均以提交后的**最终形态**（目标等级、目标转职/觉醒档位）校验 §3.3 门槛——既约束"低等级设高觉醒"，
也约束"高觉醒角色把等级改至门槛之下"。因此参考等级 `refLevel` 取目标等级时，最终觉醒档位取
「启用转职/觉醒则用下拉值，否则沿用角色当前值」。

仅改名（等级与转职/觉醒均不变化）不受等级门槛约束，允许放行——改名字不改变角色等级与成长形态。

## 4. UI 行为设计（SendItemView.vue）

页面采用**标准 header / side / main 三区布局**：

- **顶部 header（通栏）**：宽度 100% 撑满容器；左侧品牌标题，右侧承载网关状态徽标与「返回登录」。
- **左侧 side**：「收件目标」面板（账号 / 管理密钥 / 查询角色 / 收件角色选择 / 清空邮件）——即查询账号角色入口，宽度固定（340px），始终可见。
- **右侧 main**：主体区 100% 自适应撑满剩余宽度，其余功能卡片纵向排布——「修改角色」面板、「邮件附件」面板、底部操作栏。

面板行为：

1. 「修改角色」面板复用侧栏的账号 / 密钥 / 收件角色选择上下文；未查询角色时面板呈空态提示。
...
4. 等级门槛按 §3.3/§3.4 双向校验：启用转职/觉醒组时检查目标形态；仅启用设等级而角色当前
   转职/觉醒形态超出目标等级门槛时同样拦截，均以块状提示展示拦截原因并引导调整。
5. 提交前 confirm 弹窗汇总待变更项，并固定提示：仅离线角色生效、实际发生等级或转职/觉醒变更会清空已学技能（下次选角自动重建）。
6. 成功后自动重查角色列表刷新下拉标签（新名字 / 新等级即时可见），并以 alert 展示各项 `*_updated` 标志与最终值。
7. 角色标签展示完整成长形态：`角色名 (Lv等级 / 基础职业 · 转职·觉醒)`。基础职业取 `RoleInfo.job`——
   proto3 `int32` 默认值 0 不序列化，解码后字段缺失时按 0（鬼剑士男）回退处理；转职/觉醒名由
   `grow_type` 解码（`(second<<4)|first`）并经职业枚举表（`src/utils/jobGrowNames.js`）解析，
   未知分支回退泛化档位文案。未转职显示「未转职」，未觉醒不附加觉醒名。
...
- 2026-08-27 布局与规则调整：① 页面改标准 header/side/main 三区——顶部通栏菜单区（预留）、
  左侧「收件目标」查询账号角色面板、右侧「修改角色」+「邮件附件」+ 操作栏（§4 已改写）；
  ② 等级门槛按 86 版本运营口径更新为转职 15 / 一觉 50 / 二觉 75（§3.3），并新增等级与觉醒
  双向约束（§3.4）：仅改名放行，改等级或转职/觉醒任一变化均以最终形态校验门槛。
  验证：`test/update-role-proto.mjs` 24 PASS / 0 FAIL、`npx vite build` 构建通过（协议层未变，
  展示层规则与布局经人工对照页面检查）。
- 2026-08-27 角色标签补全：`RoleInfo.job` 为 proto3 int32，网关侧 job=0（如鬼剑士男）不序列化，
  前端解码后字段缺失被 `jobName` 回退为「鬼剑士(男)」，下拉标签此前仅显示基础职业、不显示转职/觉醒。
  于 `roleLabel` 追加 grow_type 解码并经 `jobGrowNames` 枚举表解析完整成长形态
  （`角色名 (LvN / 基础职业 · 转职·觉醒)`，未知分支回退泛化文案），§4 新增第 7 条行为说明。
  验证：`npx vite build` 构建通过；收件角色下拉与修改角色面板当前值展示一致。
