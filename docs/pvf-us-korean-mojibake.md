...
- 存储链路（双重转码）：
  ...
  ```
- 例：sTrW 首条原始字节 `63 25 18 25 24 25 0c 25 93 25 bf 00 34 25 7f 20` = UTF-16LE 的 `╣┘┤┌▓¿┴ⁿ`，其中 `╣`(U+2563)=CP437 0xB9、`┘`(U+2518)=0xD9、`┤`(U+2524)=0xB4、`┌`(U+250C)=0xDA、`▓`(U+2593)=0xB2、`¿`(U+00BF)=0xA8、`┴`(U+2534)=0xC1、`ⁿ`(U+207F)=0xFC。
...
修复后 `itemname.lst` 样本：`` `(구)1 ~ 10레벨 가죽 방어구 레가시` 7012 ``、`` `(구)1848 드라군` 31035 ``；`monstername.lst` 样本：`` `고블린` 1 ``、`` `고블린 투척병` 2 ``。
...
| 检查项 | 结果 |
|--------|------|
| sTrW 全表回归 | 变更全部为韩文恢复，**非韩文变更 0** |
| sTrA 全表回归 | **变更 0**（`ぃ` 等日文假名保持原样） |
| itemname.lst | 行内名字恢复为正确韩文；用户样本码全部还原 |
| monstername.lst | 名字恢复韩文（`고블린`、`고블린 투척병`） |
